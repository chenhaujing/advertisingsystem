...
public class StoreCategoryStandardApiController extends BaseApiController {
    ...
    @ApiOperation(value = "创建门店类别标准", notes = "创建人: 张瑞兵")
    @PostMapping(value = "create")
    public InvokeResult createStoreCategoryStandard(@Validated @RequestBody CreateStoreCategoryStandardViewModel viewModel, BindingResult result) {
    ...
    public InvokeResult<PageList<StoreCategoryStandardDetailViewModel>> queryStoreCategoryStandards(@Validated @RequestBody StoreCategoryStandardPageInfoViewModel viewModel, BindingResult result) {
        if (result.hasErrors())
            return ValidateFailResult(result);
        Pageable pageable = new MyPageRequest(viewModel.getPageIndex(), viewModel.getPageSize(),new QSort(QStoreCategoryStandard.storeCategoryStandard.storeType.asc(),QStoreCategoryStandard.storeCategoryStandard.commodityAmountMax.asc()));
    ...
    private void validateStoreCategoryStandardViewModel(CreateStoreCategoryStandardViewModel viewModel){
        ...
        if(viewModel.getAvgDailyTradingAmountMin()>=viewModel.getAvgDailyTradingAmountMax())
            throw new BusinessException("平均每日交易订单数最大值不能小于等于最小值！");

        if(viewModel.getCommodityAmountMin()>=viewModel.getCommodityAmountMax())
            throw new BusinessException("商品数量最大值不能小于等于最小值！");
