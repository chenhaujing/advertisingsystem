...
public class PdfUtils {
     ...
     */
    public static void createPdfFromTemplate(PreviewConfig previewConfig,JSONObject content) throws IOException, DocumentException, JSONException {
        ...
        if(previewConfig.getPkgtype().equals(Constant.WAR)){
            String templatePath= FileUtils.getAssertsBasePath(previewConfig.getPkgtype())+previewConfig.getTempaltepath();
            reader=new PdfReader(templatePath);
        }else{
            InputStream stream = new FileUtils().getClass().getClassLoader().getResourceAsStream(previewConfig.getTempaltepath());
        ...
        for (Map.Entry<String, Object> entry : content.entrySet()) {
            ...
            acroFields.setField(key,entry.getValue().toString());
        }
    ...
    private static void  writeTable(Document document,PreviewConfig previewConfig,JSONObject content) throws DocumentException {
        ...
        for(int keyIndex=0;keyIndex<key.length;keyIndex++){
            ...
            if(keyname.equals("playinfo")){
                for(int j=0;j<jsonKeys.length;j++){
                    table.addCell(new Paragraph(content.getString(jsonKeys[j]),font));
                }
            }
            document.add(table);
            document.newPage();
